...
for (int i = 0; i < mid; i++) {
    ...
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            int idx = i * 16 + r * 4 + c;
            if (idx < len) {
                plaintext_table[c][r] = (uint8_t)plaintext[idx];
            } else {
                plaintext_table[c][r] = (uint8_t)pad_value;
            }
        }
    }
    ...
    uint8_t state[4][4];
    uint8_t tmp1[4][4], tmp2[4][4], tmp3[4][4];
...
uint8_t text_for_decrypting[data_len];
...
uint8_t temp2[4][4];
uint8_t temp3[4][4];
uint8_t decrypted_text[data_len];
...
if(auth==true) {
    ...
    memcpy(IV_key_copied, final_result, sizeof(IV_key_copied));
    ...
    for (int i = 0; i < blocks_count; i++) {
        ...
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                block[c][r] = text_for_decrypting[i * 16 + r * 4 + c];
            }
        }
        ...
        // XOR з попереднім шифротекстом (CBC)
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
        ...
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                decrypted_text[i * 16 + r * 4 + c] = state_decrypt[c][r];
            }
        }
    ...
    // Перевірка падінгу
    uint8_t padding_value = decrypted_text[(data_len - 16) - 1];
...
printf("\n");
    ...
    if (padding_value < 1 || padding_value > 16 || padding_value > data_len) {
        valid_padding = false;
    } else {
        for (size_t i = 0; i < padding_value; i++) {
            if (decrypted_text[(data_len - 16) - 1 - i] != padding_value) {
                valid_padding = false;
                break;
            }
        }
    ...
    size_t decrypted_len = data_len - padding_value;
...
int main() {
    ...
    int len = strlen(plaintext);
    int pad_value = (len % 16 == 0) ? 16 : 16 - (len % 16);
    int mid = (len / 16) + 1;  // always add padding block
